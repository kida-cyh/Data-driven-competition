...
try:
    ...
    for index, row in merged_df.iterrows():
        ...
        # 질병명  받침글자가 있는지 확인하여 은는 결정
        last_char = disease_name[-1]  # 마지막 글자 추출
        is_jongseong = (ord(last_char) - 44032) % 28 > 0  # 받침 여부 판단
        suffix = '은' if is_jongseong else '는'
...
#JSON 파일로 저장
with open('opendata_qna_list_final.json', 'w', encoding='utf-8') as file:
