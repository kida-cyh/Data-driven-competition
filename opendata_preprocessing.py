...
try:
    ...
    merged_df = pd.merge(merged_df, most_common_genders, on='주상병명')
    ...
    for index, row in merged_df.iterrows():
        ...
        # 답변 추출         
        answer = f"2019년 기준으로 {disease_name}는 {disease_frequency}명의 환자가 발생하였습니다. 전체 환자 중 {gender_percentage}%가 {gender}환자이며, 연령대 별로는 {age}가 지역별로는 {location}지역에서 가장 많은 환자가 보고되었고, 월별로는 {month}월에 {month_frequency}명으로 발병이 가장 많았습니다. 이 질병의 치료기간은 평균{average_stay}일입니다. "
...
#JSON 파일로 저장
with open('opendata_qna_list.json', 'w', encoding='utf-8') as file:
